package aqario.fowlplay.common.entity.ai.brain.forge;
...
public class ExtendedBrain<E extends LivingEntity & ExtendedBrainOwner<E>> extends SmartBrain<E> {
    ...
    public ExtendedBrain(E owner, List<MemoryModuleType<?>> memories, List<? extends ExtendedSensor<E>> sensors, @Nullable List<BrainActivityGroup<E>> taskList) {
        super(owner, memories, sensors, null, false);
        this.owner = owner;
    ...
    public BrainBuilder<E> createBuilder() {
        final ExtendedBrain<E> brain = this;

        final BrainBuilder<E> builder = new BrainBuilder<>(this) {
            @Override
            public Provider<E> provider() {
                return new SmartBrainProvider<>(ExtendedBrain.this.owner);
            }
    ...
    private void findAndSetActiveActivity(E entity) {
        Activity nextActivity = this.getFirstValidActivity(entity.getActivityPriorities());

        if(nextActivity != null && entity.getScheduleIgnoringActivities().contains(nextActivity)) {
            this.setActiveActivity(nextActivity);
            return;
        }
        ...
        if(this.schedule != null) {
            ...
            if(scheduledActivity != null && this.activityRequirementsAreMet(scheduledActivity)) {
                if(!this.isActive(scheduledActivity)) {
    ...
    public Brain<E> copyWithoutBehaviors() {
        ExtendedBrain<E> brain = new ExtendedBrain<>(this.owner, this.memories.keySet().stream().toList(), this.sensors.stream().map(pair -> (ExtendedSensor<E>) pair.getSecond()).toList(), null);
    ...
    private static <E extends LivingEntity> void consumeBehaviour(int priority, Activity activity, BehaviorControl<E> behaviour, @Nullable BehaviorControl<E> parentBehaviour, BrainBehaviourConsumer consumer) {
        ...
        if(behaviour instanceof GateBehavior<E> groupBehaviour) {
            groupBehaviour.behaviors.stream().forEach(childBehaviour -> consumeBehaviour(priority, activity, (BehaviorControl<E>) childBehaviour, groupBehaviour, consumer));
        }
