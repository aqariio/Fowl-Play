...
public class TeleportToTarget extends ExtendedBehaviour<BirdEntity> {
    ...
    private boolean canTeleportTo(BirdEntity entity, BlockPos pos) {
        BlockPathTypes type = WalkNodeEvaluator.getBlockPathTypeStatic(entity.level(), pos.mutable());
        if(type != BlockPathTypes.WALKABLE && type != BlockPathTypes.OPEN && type != BlockPathTypes.LEAVES) {
            return false;
        }
